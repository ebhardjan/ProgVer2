...
object supportedViperSyntax {
  ...
  def isSupportedMethod(method : sil.Method) : Boolean = method match {
    case sil.Method(name, formalArgs, formalReturns, preconditions, postconditions, locals, body) =>
      formalArgs.forall(v => isSupportedType(v.typ)) &&
        ...
        locals.forall(v => isSupportedType(v.typ)) &&
        isSupportedStatement(body)
  }
  ...
  def isSupportedStatement(statement: sil.Stmt) : Boolean = statement match {
    ...
    case Seqn(statements) => // Viper sequential composition takes a Scala Seq of Stmt elements
      statements.forall(stm => isSupportedStatement(stm))
    case While(condition, invariants, locals, body) =>
      isSupportedExpression(condition) && invariants.forall(inv => isSupportedExpression(inv)) && locals.forall(v => isSupportedType(v.typ)) && isSupportedStatement(body)
